...
string DataConverter::Rtrim(string s)
{
...
vector<int> DataConverter::SplitStringToInts(
	string str, char delimeter)
{
	string current;
	string previous;

	string copy = Rtrim(str);
	...
	for( int i = 0; i < copy.length(); i++)
	{
		if( copy[i] == delimeter )
		{
			integers.push_back(GetStringToInt(current));
			previous = current;
			current = string("");
		}
		else
