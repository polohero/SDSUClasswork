...
#define EVEN_THREAD_SLEEP 2
#define ODD_THREAD_SLEEP 3
...
int main()
{
	...
	startThreads(threads, THREAD_COUNT);
...
void logPid()
{
	...
	string pidMessage =
		string("Process ID: ") +
		DataConverter::GetIntToString(getpid());
...
void writeThreadData(thread_data data)
{
	...
	FileHelper::WriteLine(SHARED, Thread::GetCurrentThreadID());
	cout << "Thread " << Thread::GetCurrentThreadID() << " is running " << endl;
